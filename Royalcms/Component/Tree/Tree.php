<?php namespace Royalcms\Component\Tree;
...
class Tree {
    ...
    public function get_tree_array2($myid, $str, $sid = 0, $adds = '', $str_group = '',$first=0,$class='tree treeFolder') {
        $number = 1;
        $child = $this->get_child($myid);
        //dump($child);exit;
        if (is_array($child)) {
            ...
            foreach ($child as $id => $value) {
                ...
                $this->arr_tree .= $nstr;
                $nbsp = $this->nbsp;
                $re =  $this->get_tree_array2($id, $str, $sid, $adds . $k . $nbsp, $str_group,1);
                if(empty($re)){
                    $this->arr_tree .= '</li>';
                }  else {
                    $this->arr_tree .= '</ul>';
